...
public class BijectionVer2 {
	...
	private int firstnumber(String str, int number) {
	...
	public static void main(String[] args) {
        ...
        int n = str.length(); 
        char[] src = str.toCharArray();
        BijectionVer2 testBijection = new BijectionVer2();
    ...
    private void Bijection(String str) {
		...
		for (int i=1;i<leng;i++) {
			if (inte[i] < inte[i-1]) { // increase by n
				firsttwo = firstnumber(makString,inte[i]);
				...
				if (inte[i-1] == greatest) {
					firstone = 0;
				} else if (firstone < greatestposi) {
					firstone ++;
				}
				makString = makString.substring(0, firstone+1) + inte[i] + makString.substring(firstone+1,makString.length());	       
				//makString = inte[i] + makString;
				firstone++;
...
//				makString = new String(mak123);
				...
				if (inte[i] > greatest) {
					...
					char[] mak123 = makString.toCharArray();
					if (firsttwo <= greatestposi) {
						greatestposi++;
					}
					for (int j=firsttwo+1;j < makString.length(); j++) {
						if (Character.getNumericValue(mak123[j]) == inte[i]){
							makString = makString.substring(0, firsttwo) + inte[i] + makString.substring(firsttwo,j) + makString.substring(j+1,makString.length());	       
							mak123 = makString.toCharArray();
							if (firsttwo <= greatestposi) {
								greatestposi++;
							}
						}
